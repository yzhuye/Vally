...
class GroupRepositoryImpl implements GroupRepository {
  ...
  @override
  bool assignStudentToGroup(
      String courseId, String groupId, String studentEmail) {
    final groupHive = _box.get(groupId);
    if (groupHive == null) return false;
    ...
    final allGroups = _box.values
        .where((g) =>
            g.courseId == courseId && g.categoryId == groupHive.categoryId)
        .toList();

    for (final otherGroup in allGroups) {
      if (otherGroup.id != groupId &&
          otherGroup.members.contains(studentEmail)) {
  ...
  @override
  bool moveStudentToGroup(String courseId, String fromGroupId, String toGroupId,
      String studentEmail) {
    // Primero remover del grupo origen
    final fromGroupHive = _box.get(fromGroupId);
    if (fromGroupHive == null ||
        !fromGroupHive.members.contains(studentEmail)) {
      return false;
    }
    ...
    if (toGroupHive == null ||
        toGroupHive.members.length >= toGroupHive.maxCapacity) {
      return false;
    }
  ...
  @override
  Group? findStudentGroup(
      String courseId, String categoryId, String studentEmail) {
    try {
      final groups = _box.values.where(
        (g) =>
            g.courseId == courseId &&
