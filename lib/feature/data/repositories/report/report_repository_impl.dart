...
import 'package:vally_app/feature/data/repositories/activity/activity_repository_impl.dart';
import 'package:vally_app/feature/data/repositories/evaluation/evaluation_repository_impl.dart';
import 'package:vally_app/feature/data/repositories/group/group_repository_impl.dart';
import 'package:vally_app/feature/data/repositories/course/category_repository_imp.dart';
import 'package:vally_app/core/services/user_service.dart';
...
class ReportRepositoryImpl implements ReportRepository {
  ...
  Future<ReportData> generateReportData({
    ...
    // Get category name
    final categories =
        await _categoryRepository.getCategoriesByCourse(courseId);
    ...
    for (final group in groups) {
      for (final studentId in group.members) {
        ...
        studentReports.add(StudentAverageReport(
          studentId: studentId,
          studentName:
              _getStudentName(studentId), // You'll need to implement this
          overallAverage: totalScoreSum / (evaluationCount * 4),
          punctualityAverage: punctualitySum / evaluationCount,
          contributionsAverage: contributionsSum / evaluationCount,
          commitmentAverage: commitmentSum / evaluationCount,
    ...
    return groups.map((group) {
      ...
      for (final studentId in group.members) {
        ...
        students.add(StudentDetailedResult(
          studentId: studentId,
          studentName: _getStudentName(studentId),
  ...
  CriteriaScore _createCriteriaScore(
    ...
    final individualEvaluations = evaluations.map((e) {
      ...
      return IndividualEvaluation(
        ...
        evaluatorName: _getStudentName(e.evaluatorId),
  ...
  String _getStudentName(String studentEmail) {
    // Helper method to map emails to student names using API
    final username = UserService().getUsernameByEmail(studentEmail);
    if (username != null) {
  ...
  Future<List<Group>> getGroupsByCategory({
    ...
    return _groupRepository.getGroupsByCategory(courseId, categoryId);
  }
}
